...
!mkdir data
!mv imagenetmini-1000.zip data/

!unzip data/imagenetmini-1000.zip
...
#experiments = ['vit']
augmentations = ['abc'] #TODO: get the list from config, and apply for each image
for experiment in experiments:
  for augmentation in augmentations:  # TODO: replace
    ...
    trainer.test(net)
    #net.on_save_checkpoint(experiment_name+"_trained.ckpt")
...
! ls /content/drive/MyDrive/Colab\ Notebooks/dlfa-capstone/run2-tb-logs/logs/deit_imagenet_abc_aa_rc_cm_mu_gap/version_0

! cp -R logs/ /content/drive/MyDrive/Colab\ Notebooks/dlfa-capstone/run2-tb-logs/

! ls

# code for image augmentation - just to try the test process algo..may need to replace with Amrit/Bala's version

def createaugimages(dirname,augment_name):  ## may not need this fn, as may directly work with tensor from transform
    ...
    for image in filename:
        ...
        augment_dir = augment_name+"/"+dirname 
        isDirExist = os.path.exists(augment_dir)
        if not isDirExist:
          # Create a new directory because it does not exist
          os.makedirs(augment_dir)
      
        newimagepath = augment_dir +"/"+ image
...
def predict(model, test_image_name,transform,image_size):
...
augimgtransform = transforms.Compose([ 
...
folder_path = 'imagenet-mini/train'
subdirectories = [f.name for f in os.scandir(folder_path) if f.is_dir()]
print(subdirectories)

pred_dir = os.path.join("./aug1/imagenet-mini/", "train")
...
pred_dl = torch.utils.data.DataLoader(pred_dataset, batch_size=1, num_workers=args.num_workers, pin_memory=True)

a1 = parser.parse_args(args=['--api-key','','--dataset', 'imagenet', '--num-classes', '1000','--model-name','vit','--patch','8','--batch-size', '64', '--eval-batch-size','256','--lr','1e-3','--min-lr', '1e-5','--beta1', '0.9','--beta2', '0.999','--off-benchmark', '--max-epochs', '2','--dry-run', '--weight-decay', '5e-5', '--warmup-epoch', '5', '--precision', '16','--autoaugment', '--criterion', 'ce', '--smoothing', '0.1','--rcpaste', '--cutmix', '--mixup', '--dropout', '0.0','--head', '12','--num-layers','7','--hidden','384','--mlp-hidden','384', '--off-cls-token', '--seed','42','--project-name', 'VisionTransformer'])
a1.model_name = 'vit'
a1.size = 224
a1.augment = augmentation
a1

mdl_vit = Net.load_from_checkpoint("/content/logs/vit_imagenet_abc_aa_rc_cm_mu_gap/version_0/checkpoints/epoch=0-step=543.ckpt",hparams=a1)

no_dir_to_check = 1
#df_report = pd.DataFrame(columns=['File_Name', 'Actual_Label', 'Predicted_VIT_Label','Predicted_DEIT_Label','Predicted_SWIN_Label'])
df_report = pd.DataFrame(columns=['File_Name', 'Actual_Label', 'Predicted_VIT_Label'])

# load above trained model 

#mdl_deit = Net.load_from_checkpoint("/content/logs/deit_imagenet_abc_aa_rc_cm_mu_gap/version_1/checkpoints/epoch=0-step=543.ckpt")
#mdl_swin = Net.load_from_checkpoint("/content/logs/swin_imagenet_abc_aa_rc_cm_mu_gap/version_1/checkpoints/epoch=0-step=543.ckpt")
...
for s in subdirectories[:no_dir_to_check]:
  folder_path='imagenet-mini/train/'+s
  files = [f.path for f in os.scandir(folder_path) if f.is_file()]
  #createaugimages(folder_path,"aug1") ## not needed as using tensor from transform..saved image though is useful for debug/visualization
  for f in files:
    print(f)
    fname = f
    org_img = Image.open("aug1/"+f)
    
    img_arr = np.array(org_img.convert("RGB"))
    trans = transforms.ToTensor()
    tensor = trans(org_img)
    tensor = tensor.unsqueeze(0)
    with torch.no_grad():
      #p = mdl_vit.model(img_arr)
      p = mdl_vit.model(tensor)
      #p = trainer.predict(mdl_vit, pred_dl)
      vit_pred_class = maxtensorval(p)
      vit_class = vit_pred_class.item()
      df_report = df_report.append({'File_Name': fname, 'Actual_Label': 'tbd', 'Predicted_VIT_Label': vit_class}, ignore_index=True)
    break
    #image_f_path,vit_pred_tensor = predict(mdl_vit, f,augimgtransform,224)
    #image_f_path,deit_pred_tensor = predict(mdl_deit, f,augimgtransform,224)
    #image_f_path,swin_pred_tensor = predict(mdl_swin, f,augimgtransform,224)
    #vit_pred_class = maxtensorval(vit_pred_tensor)
    #deit_pred_class = maxtensorval(deit_pred_tensor)
    #swin_pred_class = maxtensorval(swin_pred_tensor)
    #vit_class = vit_pred_class.item()
    #deit_class = deit_pred_class.item()
    #swin_class = swin_pred_class.item()

    #df_report = df_report.append({'File_Name': fname, 'Actual_Label': 'tbd', 'Predicted_VIT_Label': vit_class,'Predicted_DEIT_Label': deit_class,'Predicted_SWIN_Label': swin_class}, ignore_index=True)
