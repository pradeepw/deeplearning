...
## Load Tensorboard and install packages
"""

# Commented out IPython magic to ensure Python compatibility.
# %load_ext tensorboard

! pip install -q kaggle timm pytorch-lightning transformers

! pip -qq install git+https://github.com/ildoonet/pytorch-gradual-warmup-lr.git

"""## Upload kaggle key to be used for downloading mini imagenet data set from kaggle"""
...
"""## Download imagenetmini-1000"""

! kaggle datasets download -d ifigotin/imagenetmini-1000

!mkdir data
!mv imagenetmini-1000.zip data/
!unzip -qq data/imagenetmini-1000.zip
...
"""## Utility classes and function for augmentation"""
...
"""## Code to randomly apply some augmentation for imagenet and cifar dataset during training.."""
...
"""## Get pretrained model for VIT, SWIN and DeIT along with related dataset"""
...
def get_model(args):
    if args.model_name == 'vit':
        net = timm.create_model('vit_base_patch16_224',pretrained=False,num_classes=args.num_classes, img_size=224) 
        ## TODO: get the transformer from timm for the model
...
"""## Various parameters that can be changed before starting the program run. An experiment can be considered one combination of these parameter that are set while running the program"""
...
args = parser.parse_args(args=['--api-key','','--dataset', 'imagenet', '--num-classes', '1000','--model-name','swin','--patch','8','--batch-size', '128', '--eval-batch-size','256','--lr','1e-4','--min-lr', '1e-5','--beta1', '0.9','--beta2', '0.999','--off-benchmark', '--max-epochs', '200','--dry-run', '--weight-decay', '5e-5', '--warmup-epoch', '5', '--precision', '16','--autoaugment', '--criterion', 'ce', '--smoothing', '0.1','--rcpaste',  '--dropout', '0.0','--head', '12','--num-layers','7','--hidden','384','--mlp-hidden','384', '--off-cls-token', '--seed','42','--project-name', 'VisionTransformer'])
...
"""## Preparing the dataloader for imagenetmini"""
...
#for cifar - dataset available & dont need explicit download, no resize
...
# for miniimagenet - due to need for downloading & placing data resize
...
"""## Define the pytorch lightning model. (will have one of the VIT/SWIN/DEIT as the underlying model)"""
...
"""## Run the experiment for model and selected augmentation..the names here are lable manally set for now"""

#experiments = ['deit', 'swin','vit']
experiments = ['swin']
augmentations = ['le4b128e100'] 
for experiment in experiments:
  for augmentation in augmentations:  
    ...
    refresh_rate = 1
    net = Net(args)
    ModelSummary(net, max_depth=-1)
    trainer = pl.Trainer(precision=args.precision,fast_dev_run=False, gpus=args.gpus, benchmark=args.benchmark, logger=logger, max_epochs=args.max_epochs,callbacks=[ModelCheckpoint(save_weights_only=True, mode="max", monitor="val_acc"),LearningRateMonitor("epoch"),],log_every_n_steps=50)
    trainer.fit(net)
...
! mkdir -p /content/drive/MyDrive/Colab\ Notebooks/dlfa-capstone/run-swin-le4b128e200/logs/

! cp -R logs/ /content/drive/MyDrive/Colab\ Notebooks/dlfa-capstone/run-swin-le4b128e200/

# Commented out IPython magic to ensure Python compatibility.
# %tensorboard --logdir '/content/drive/MyDrive/Colab Notebooks/dlfa-capstone/run-deit-le4b128e200-logs/'

# Commented out IPython magic to ensure Python compatibility.
# %tensorboard --logdir '/content/drive/MyDrive/Colab Notebooks/dlfa-capstone/run-swin-le4b128e200/'

# Commented out IPython magic to ensure Python compatibility.
# %tensorboard --logdir '/content/drive/MyDrive/Colab Notebooks/dlfa-capstone/run-vit-le4b128e200/'